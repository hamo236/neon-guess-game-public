...
if (!gameplay.includes('disabled={!myTeamRequired || !actions.targetReady || !actions.canMutateCompetitive || (hasConfirmed && confirmedIds.length === 2)}')) throw new Error('Confirmation must remain restricted to the authoritative defending team, current target readiness, mutable state, and immutable completed pair.');
