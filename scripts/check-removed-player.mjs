...
const footballIds = remainingFootballIds.filter((id) => Number(id) >= 1 && Number(id) <= 29);

for (const token of removedTokens) {
  if (source.includes(token)) throw new Error(`Removed player regression: found ${token}`);
}

if (footballIds.length !== 28) {
  throw new Error(`Football dataset regression: expected 28 players, found ${footballIds.length}`);
}
...
console.log('Removed-player regression passed: Nico Schlotterbeck/f27 is absent; 28 football players remain; f26 and f28 are intact.');
